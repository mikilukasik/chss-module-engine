...
export function getDefenderValues(board: Uint8Array, cellIndex: i8, color: u8): i16[] {
  const defenderValues: i16[] = [];
  const rank: u8 = cellIndex >>> 3;

  // check if attacked by pawn
  if (color === 0) {
    if (rank < 6) {
      if (board[cellIndex + 7] === 9) defenderValues[defenderValues.length] = 100;
      if (board[cellIndex + 9] === 9) defenderValues[defenderValues.length] = 100;
    }
  } else {
    if (rank > 1) {
      if (board[cellIndex - 7] === 1) defenderValues[defenderValues.length] = 100;
      if (board[cellIndex - 9] === 1) defenderValues[defenderValues.length] = 100;
    }
  }
  ...
  for (let i: u8 = 0, len = u8(lookForKnightOn.length); i < len; i += 1)
    if ((board[lookForKnightOn[i]] & 7) === 3) defenderValues[defenderValues.length] = 305;
  ...
  for (let currentCellIndex: i8 = cellIndex - 9; currentCellIndex >= lastCellUpLeft; currentCellIndex -= 9) {
    ...
    if (board[currentCellIndex] === defendingBishop) defenderValues[defenderValues.length] = 333;
    if (board[currentCellIndex] === defendingQueen) defenderValues[defenderValues.length] = 950;
    if (board[currentCellIndex] === defendingKing && currentCellIndex === cellIndex - 9)
      defenderValues[defenderValues.length] = 1500;
    break;
  }
  ...
  for (let currentCellIndex: i8 = cellIndex - 8; currentCellIndex >= lastCellUp; currentCellIndex -= 8) {
    if (board[currentCellIndex] === 0) continue; // empty cell
    if (board[currentCellIndex] === defendingRook) defenderValues[defenderValues.length] = 563;
    if (board[currentCellIndex] === defendingQueen) defenderValues[defenderValues.length] = 950;
