export const getPawnHitMoves = (position, board, isWhite) => {
  const targets = [];
  ...
  if (isWhite) {
    ...
    if (canHitLeft) {
      ...
      if (board[leftHitTarget] === 0) {
        ...
        if (rank === 1) {
          // promotion and underPromotion
          targets[targets.length] = leftHitTarget + (13 << 6); // can become rook
  ...
  if (canHitLeft) {
    ...
    if (board[leftHitTarget] === 0) {
      ...
      if (rank === 6) {
        // promotion and underPromotion
        targets[targets.length] = leftHitTarget + (5 << 6); // can become rook
